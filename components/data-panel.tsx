...
export function DataPanel({ position, title, type = "standard", audioAnalyser }: DataPanelProps) {
  ...
  useEffect(() => {
    if (type === "waveform" && waveformCanvasRef.current) {
    ...
    if (type === "standard") {
      const updateMetrics = () => {
        if (audioAnalyser) {
          ...
          // Update stability based on amplitude
          const newStabilityValue = 50 + Math.round(amplitude * 50)
          setStabilityValue(newStabilityValue)

          // Update status color
          if (newStabilityValue < 40) {
            setStatusColor("#ff00a0")
          } else if (newStabilityValue < 70) {
            setStatusColor("#ffae00")
          } else {
            setStatusColor("#ff4e42")
          }

          // Occasionally update other metrics
          if (Math.random() < 0.05) {
            setMassValue(Number.parseFloat((1 + amplitude * 2).toFixed(3)))
            setEnergyValue(`${(amplitude * 10).toFixed(1)}e8 J`)
            setVarianceValue(Number.parseFloat((amplitude * 0.01).toFixed(4)))
          }
        } else if (Math.random() < 0.05) {
          // Random fluctuations when no audio
          const randomStability = Math.floor(Math.random() * 30) + 60
          setStabilityValue(randomStability)

          setMassValue(Number.parseFloat((Math.random() * 2 + 1).toFixed(3)))
          setEnergyValue(`${(Math.random() * 9 + 1).toFixed(1)}e8 J`)
          setVarianceValue(Number.parseFloat((Math.random() * 0.01).toFixed(4)))
        }
      }
    ...
    return (
      ...
      >
        ...
        <div className="data-bar">
          <div className="data-bar-fill" style={{ width: `${stabilityValue}%` }}></div>
        </div>
        <div className="data-readouts">
          <div className="data-row">
            <span className="data-label">STABILITY INDEX:</span>
            <span className="data-value">{stabilityValue}%</span>
          </div>
          <div className="data-row">
            <span className="data-label">MASS COEFFICIENT:</span>
            <span className="data-value">{massValue}</span>
          </div>
          <div className="data-row">
            <span className="data-label">ENERGY SIGNATURE:</span>
            <span className="data-value">{energyValue}</span>
          </div>
          <div className="data-row">
            <span className="data-label">QUANTUM VARIANCE:</span>
            <span className="data-value">{varianceValue}</span>
          </div>
        </div>
      </div>
    )
    ...
    >
      ...
      <div className="data-readouts">
        <div className="data-row">
          ...
          <span className="data-value">{peakValue}</span>
        </div>
        <div className="data-row">
          <span className="data-label">AMPLITUDE:</span>
          <span className="data-value">{amplitudeValue}</span>
        </div>
        <div className="data-row">
          <span className="data-label">PHASE SHIFT:</span>
          <span className="data-value">{phaseValue}</span>
        </div>
      </div>
    </div>
  )
