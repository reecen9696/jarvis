...
interface SatelliteGlobesProps {
  scene: THREE.Scene
  options: {
    resolution: number
    distortion: number
...
export function SatelliteGlobes({ scene, options }: SatelliteGlobesProps) {
  const globesRef = useRef<THREE.Group[]>([])
  const satelliteCount = 4

  useEffect(() => {
    const satelliteGlobes: THREE.Group[] = []

    // Create satellite globes
    for (let i = 0; i < satelliteCount; i++) {
      const satelliteGroup = new THREE.Group()
      const satelliteRadius = 0.5 // Smaller than the main globe
    ...
    globesRef.current = satelliteGlobes
